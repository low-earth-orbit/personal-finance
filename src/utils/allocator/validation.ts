import type { AllocatorErrors, AllocatorInput, AllocatorInputKey } from "./types";
...
export function validateAllocatorInput(input: AllocatorInput): AllocatorErrors {
  ...
  for (const [key, constraint] of Object.entries(FIELD_CONSTRAINTS) as [
    ...
    } else if (value < constraint.min || (constraint.max != null && value > constraint.max)) {
      errors[key] =
        constraint.max == null
  ...
  if (
    ...
    errors.distributionYieldPct = "Distribution yield cannot exceed nominal return.";
  }
  return errors;
}
